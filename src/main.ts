...
import { MarkdownView, Notice, Plugin, PluginSettingTab, TFile, type App, type ObsidianProtocolData, type SettingDefinitionItem } from 'obsidian';
import { requestUrl } from 'obsidian';
import { RssApi } from './api';
import { t } from './i18n';
import { articleProtocol, pluginId } from './plugin-id';
import { folderPath, initialState, modeLabels, readingModes, modeSchema, readingFontSchema, type Bundle, type Entry, type Mode, type State } from './model';
...
export default class QiaomuRssPlugin extends Plugin {
  ...
  async onload() {
    ...
    catch { new Notice(t.configIncompatible); }
    ...
    this.addCommand({ id: 'manage-subscriptions', name: t.manageSubscriptions, callback: () => this.manageSubscriptions() });
    ...
    this.addCommand({ id: 'explore-subscriptions', name: t.exploreSubscriptions, callback: () => { void this.openDiscovery(); } });
    this.addRibbonIcon('rss', t.openReader, () => { void this.openReader(); });
    this.addCommand({ id: 'open-reader', name: t.openReader, callback: () => { void this.openReader(); } });
    ...
    this.registerMarkdownPostProcessor(element => {
      for (const link of element.querySelectorAll<HTMLAnchorElement>('a[href^="obsidian://vietnam-ai-rss?"], a[href^="obsidian://qiaomu-ai-rss?"]')) {
    ...
    const registerLinks = (doc: Document) => this.registerDomEvent(doc, 'click', event => {
      ...
      void this.openSavedArticle(url.searchParams.get('article') || '', url.searchParams.get('mode') || 'original')
        .catch(() => new Notice(t.savedCopyMissingLink));
    }, { capture: true });
    registerLinks(document);
    this.registerEvent(this.app.workspace.on('window-open', (_window, win) => registerLinks(win.document)));
    const openFromProtocol = (params: ObsidianProtocolData) => {
      void this.openSavedArticle(params.article || '', params.mode || 'original').catch(() => new Notice(t.savedCopyMissing));
    };
    this.registerObsidianProtocolHandler(pluginId, openFromProtocol);
    this.registerObsidianProtocolHandler('qiaomu-ai-rss', openFromProtocol);
  }
  onunload() { this.fonts.dispose(); }
  translateRequest(url: string): Promise<{ status: number; text: string }> {
    return requestUrl({ url, method: 'GET', throw: false }).then(response => ({ status: response.status, text: response.text }));
  }
  api(): RssApi {
  ...
  async openReader() {
    ...
    } catch { new Notice(t.cannotOpenReader); }
  }
  ...
  private cleanNoteMarkers(file: TFile) {
    ...
    }).catch(() => { new Notice(t.markerCleanupFailed); });
  }
  ...
  async appendToDailyNote(entry: Entry, excerpt = '', mode: Mode = 'original', target?: TFile): Promise<{ file: TFile; added: boolean }> {
    ...
    const write = async () => {
      ...
      if (target && this.app.vault.getAbstractFileByPath(target.path) !== target) throw new Error(t.noteMissing);
      ...
      let added = false;
      if (existing && !(existing instanceof TFile)) throw new Error(t.dailyPathOccupied);
      ...
      if (!(existing instanceof TFile)) throw new Error(t.cannotCreateDaily);
      if (!added) {
        const view = this.app.workspace.getLeavesOfType('markdown').map(leaf => leaf.view)
...
class RssSettings extends PluginSettingTab {
  private section = t.settingsReading;
  ...
  getSettingDefinitions(): SettingDefinitionItem[] {
    ...
    const definitions: SettingDefinitionItem[] = [
      { type: 'group', heading: t.settingsReadingExcerpt, items: [
        { name: t.selectionPopup, desc: t.selectionPopupDesc, render: setting => {
        ...
        { name: t.bodyFont, render: setting => { setting.addDropdown(drop => {
        ...
        { name: t.deviceFontName, desc: t.deviceFontDesc, visible: () => settings.fontFamily === 'custom', render: setting => { setting.addText(text => text.setPlaceholder(t.deviceFontPlaceholder).setValue(settings.customFont).onChange(async value => { settings.customFont = value.slice(0, 200); await saveReading(); })); } },
        { name: t.fontSize, render: setting => { setting.addDropdown(drop => {
          for (let size = 14; size <= 32; size++) drop.addOption(String(size), size + ' ' + t.px);
          drop.setValue(String(settings.fontSize)).onChange(async value => { settings.fontSize = Number(value); await saveReading(); });
        }); } },
        { name: t.lineHeight, render: setting => { setting.addDropdown(drop => {
          for (let value = 15; value <= 24; value++) drop.addOption((value / 10).toFixed(1), (value / 10).toFixed(1) + ' ' + t.times);
          drop.setValue(settings.lineHeight.toFixed(1)).onChange(async value => { settings.lineHeight = Number(value); await saveReading(); });
        }); } },
        { name: t.measure, render: setting => { setting.addDropdown(drop => {
          for (const width of [28, 36, 44]) drop.addOption(String(width), width + ' ' + t.chars);
      ...
      { type: 'group', heading: t.vaultMarkdown, items: [
        { name: t.readFolder, desc: t.readFolderDesc, render: setting => {
          setting.addButton(button => button.setButtonText(t.addFolder).onClick(() => {
        ...
        ...settings.markdownFolders.map(folder => ({ name: folder === '/' ? t.wholeVault : folder, render: (setting: import('obsidian').Setting) => {
          setting.addButton(button => button.setButtonText(t.remove).onClick(async () => {
      ...
      { name: t.myFeedsSetting, desc: t.myFeedsDesc, render: setting => {
        setting.addButton(button => button.setButtonText(t.manageFeeds).onClick(() => this.plugin.manageSubscriptions()));
      } },
      { name: t.opmlFolder, desc: t.opmlFolderDesc, render: setting => {
        setting.addText(text => text.setValue(settings.folder).onChange(value => { this.pendingFolder = value; }))
          .addButton(button => button.setButtonText(t.save).onClick(async () => {
            try { settings.folder = folderPath(this.pendingFolder ?? settings.folder); await this.plugin.persist(); new Notice(t.folderSaved); }
            catch (error) { new Notice(error instanceof Error ? error.message : t.cannotSaveSettings); }
          }));
      } },
      { name: t.defaultMode, render: setting => {
        setting.addDropdown(drop => {
          for (const value of readingModes) drop.addOption(value, modeLabels[value]);
      ...
      { name: t.showImages, desc: t.showImagesDesc, render: setting => {
      ...
      { type: 'group', heading: t.versionGroup, items: [
        { name: t.currentVersion(this.plugin.manifest.version), desc: t.versionDesc, render: setting => {
          setting.addButton(button => button.setButtonText(t.manageUpdates).onClick(() => this.plugin.openSettings('community-plugins')));
        } },
        { name: t.changelog, render: setting => {
          const details = setting.descEl.createEl('details');
          details.createEl('summary', { text: t.viewThisUpdate });
          details.createEl('p', { text: t.thisUpdate });
          details.createEl('a', { text: t.fullChangelog, href: 'https://github.com/mrtinhnguyen/vietnam-ai-rss/releases', attr: { target: '_blank', rel: 'noopener noreferrer' } });
        } },
      ] },
      { name: t.localData, desc: t.localDataDesc },
    ];
    const reading = definitions[0];
    if (!('type' in reading) || reading.type !== 'group') return definitions;
    const excerpt = reading.items!.shift()!;
    reading.heading = t.settingsReading;
    const buckets: Record<string, SettingDefinitionItem[]> = {
      [t.settingsReading]: [reading, definitions[4], definitions[5]],
      [t.settingsSources]: [definitions[2], definitions[1], definitions[3]],
      [t.settingsCapture]: [excerpt, definitions[7]],
      [t.settingsAbout]: [definitions[6], ...[
        [t.feedback, t.githubIssues, 'https://github.com/mrtinhnguyen/vietnam-ai-rss/issues'],
        [t.usage, t.openDocs, 'https://github.com/mrtinhnguyen/vietnam-ai-rss#readme'],
        ['X', '@vista8', 'https://x.com/vista8'],
        ['GitHub', '@mrtinhnguyen', 'https://github.com/mrtinhnguyen'],
      ...
      } })), { name: t.license, desc: t.licenseDesc }],
    };
    return [{ name: t.pluginName, searchable: false, render: setting => {
